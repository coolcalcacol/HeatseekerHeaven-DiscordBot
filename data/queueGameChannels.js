...
async function createGameChannels(gameData = new queueData.info.GameLobbyData()) {
	...
	// Send an info message explaining why the main queue channels are not visible anymore and how to gain access back
	await clientSendMessage.sendMessageTo(
		...
		[
			...
			'If you experience any problems with during this game, ask help from the `@Bot Admins` in this chat.',
		].join('\n'),
	);
	...
	// !! Doesn't cancel the game and doesn't remove the in game role
	// new botUpdate.UpdateTimer(
...
async function createVoiceChannels(gameData = new queueData.info.GameLobbyData()) {
	...
	await gameData.channels.gameChat.setTopic(gameData.channels.gameChat.topic + `_AvoiceChannels:${gameData.channels.blue.id},${gameData.channels.orange.id}_AautoQueuePlayers:${autoQueuePlayers.join(',')}`);
}
...
async function deleteGameChannels(gameData) {
	...
	await manageChannelPermissions(true, gameData);
...
async function manageChannelPermissions(reset, gameData, substituteData = { targetUser: {}, replaceUser: {}, targetTeam: '' }) {
	...
	if (reset !== 'substitute') {
		...
		await gameData.channels.gameChat.permissionOverwrites.edit(replaceUser, {
			VIEW_CHANNEL: true,
			SEND_MESSAGES: true,
			USE_APPLICATION_COMMANDS: true,
			READ_MESSAGE_HISTORY: true,
		});
		await gameData.channels[targetTeam].permissionOverwrites.edit(replaceUser, {
			CONNECT: true,
			VIEW_CHANNEL: true,
			SEND_MESSAGES: true,
			USE_APPLICATION_COMMANDS: true,
		...
		await gameData.channels[otherTeam].permissionOverwrites.edit(replaceUser, {
			CONNECT: false,
			VIEW_CHANNEL: true,
		});
