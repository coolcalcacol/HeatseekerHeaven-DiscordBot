...
let debugLog = { '[fg=green]winners[/>]': { team: {} }, '[fg=red]losers[/>]': { team: {} } };
...
function getGameResults(winningTeam, losingTeam, gameData, equationValues) {
	...
	let debugLogContentWin;
	let debugLogContentLose;
...
function calculatePlayerMmr(equationValues, combined, stats, teamMmr, gameOutcome, lobbySize) {
	...
	let result, soloBonus = 0, teamBonus = 0, totalBonus = 0;
	...
	case 'won': {
		...
		if (stats.mmr > equationValues.maxStart) { // Modify the bonus so the player doesn't reach >= maxCap (2500) mmr
	...
	case 'lost': {
		...
		totalBonus = lobbySize === 2 ? teamBonus : soloBonus * teamBonus;
		if (stats.mmr < equationValues.minStart) { // Modify the bonus so the player doesn't reach <= minCap (0) mmr
...
function equationDebug(target, equation, playerData, mode) {
	...
	debugLog[root][user][`[fg=cyan](${equation.teamBonusEq}) * (${equation.soloBonusEq})`] = '';
