...
const globalQueueData = {
    ...
    */
	...
	clearLobbyQueue(lobby) {
		const props = Object.getOwnPropertyNames(this.lobby[lobby].players);
...
class GameLobbyData {
	constructor(players, lobby) {
		...
		this.teamSelectionTotalVoteTime = 30; // in seconds
		this.captainSelectionTotalTime = 60; // in seconds
	...
	async onGameChatCreated() {
		...
		await this.getTeamSelectionMessageContent(true);
	...
	async getTeamSelectionMessageContent(send = false) {
		...
		if (this.lobby === 'threes') {
			buttonRow.addComponents(buttons.captains);
			embed.addField('Captains', `\`${embedUtilities.methods.getFieldSpacing(this.teamSelectionVotes.captains.count, 5, true)}\``, true);
		}

		if (!send) {return { content: undefined, embeds: [embed], components: [buttonRow] };}
		else if (Object.keys(this.messages.teamSelect.message).length > 0) {
	...
	async getGameRegion() {
		...
		for (const [region, { score }] of Object.entries(regionCounts)) {
			if (score > bestScore) {
				...
				const rng = generalUtilities.generate.getRandomInt(0, 1);
				const otherRegion = (rng > 0) ? region : bestRegionName;
		...
		const bestRegion = regionRoles.find(x => x.region === bestRegionName);
	...
	/* GPT:
            ...
            it sets this.
            region to a random region using to generate.
...
async function addPlayerToQueue(interaction = null, lobby, userId = null, queueSettingsData) {
	...
	if (!interaction && !userId) {
		console.log('No interaction param.');
		return;
	}
...
async function startQueue(lobby, guildId, gameData = null) {
	...
	if (await queueSettings.getQueueDatabaseById(generalData.botConfig.defaultGuildId).then((data) => {
		...
		await queueGameChannels.createGameChannels(game);
	}
	else { await clientSendMessage.sendMessageTo(game.lobbyChannelId, game.messages.queueStart.message); }
	return game;
}
// #endregion

// #region Queue Moderation
async function queueInactivityTimeout(userId) {
	...
	await queueAdmin.execute(null, true);
}
...
function getLobbyString(lobby) {
	switch (lobby) {
	case 'ones': { return '1v1'; }
	case 'twos': { return '2v2'; }
	case 'threes': { return '3v3'; }
	default: break;
	}
	return '0v0';
}
