...
async function createQueueDatabase(data) {
	thisLog('Creating new Queue database Object');
	let newData = new QueueDatabase();
...
async function getQueueDatabaseById(guildId, createIfNull = true) {
	...
	if (target) {
		...
		const output = await createQueueDatabase({ _id: guildId });
		output['_id'] = guildId;
		return output;
	}
}
...
async function getRankedLobbyById(id, guildId) {
	...
	const queueData = await QueueDatabase.findOne({ _id: guildId });
	thisLog(queueData);
	if (queueData == null) {
		thisLog('ERROR: QueueData is not defined\nat: queueDatabaseUtilities.js:12');
		return;
	}
	const channelData = queueData.channelSettings;
