...
async function updatePlayerRanks(guildId) {
	...
	const debug = false;
	let spliceStart = 0, spliceEnd = 0;
	const remainingPlayers = playerData.concat();
	for (let i = 0; i < rankData.length; i++) {
		...
		if (dist === 'top') {
			...
			//     }
			// }
			// for (let effect = 0; effect < effectedPlayers.length; effect++) {
			//     const player = effectedPlayers[effect];
			//     remainingPlayers.splice(remainingPlayers.indexOf(player), 1);
			// }
		}
...
function getRankEffectedPlayers(list, start, count, requirements) {
	const output = [];
	let validRequirements = false;
	for (const req in requirements) {
		if (requirements[req] !== '-1') {
 ...
 * @param {Boolean} createIfNull Create if the player data doesn't exist
...
async function getPlayerDataObject(userData, queueSettingsData) { // The user data that discord generates for a user
	...
	const newData = new PlayerDatabase({
		...
		userData: {
			...
			displayColor: forcedUserData.hexAccentColor ? forcedUserData.hexAccentColor : memberData ? memberData.displayHexColor : '#000000',
...
async function resetPlayerStats(interaction, reason) {
	...
	await updatePlayerRanks(interaction.guild.id);
...
async function clearPlayerData(interaction, password, reason) {
	...
	if (password !== clearPlayerDataPass) {
		...
		if (queueConfig.channelSettings.logChannel) {
			await clientSendMessage.sendMessageTo(queueConfig.channelSettings.logChannel, message);
		}
		return false;
	}
	else {
		...
		await queueSettings.updateQueueDatabase(queueConfig);
