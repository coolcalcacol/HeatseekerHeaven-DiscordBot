...
class UpdateTimer {
     ...
     * @param {Integer} time The date time in milliseconds for the timer to expire
     ...
     * @param {Integer} repeatInterval The interval in milliseconds for the timer to repeat
     */
	constructor(id, time, callback, repeat = false, repeatInterval = 1000) {
		this.id = id;
		this.time = time;
		this.callback = callback;
		this.callbackString = String(callback).replace(/\w+\s\w+\(\)\s?\{(\n\s+)?|\(\)\s?=>\s?{(\n\s+)?/, '').replace(/\n?}/, '');
