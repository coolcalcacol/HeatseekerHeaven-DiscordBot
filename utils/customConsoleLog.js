 ...
 * ◘ is a char that I use instead of more common chars to avoid errors with colorization
 */
const colors = {
	style: {
		reset: '\x1b[0m',
...
module.exports = {
     ...
     * @param {{autoColorize: boolean}} customOptions The options for the output like colorization
    */
	log(msg, customOptions = this.logOptions) {
		const options = this.logOptions;
		let output = msg;
		if (customOptions !== this.logOptions) {
			for (const defaultOptions in this.logOptions) {
				for (const inputOptions in customOptions) {
					if (inputOptions === defaultOptions) {
	 ...
	 * @param {String} indentChar The options for the output like colorization
	 * @param indentInc
	 * @param parent
	 * @param isJson
	...
	unfoldNestedObject(obj, indent, indentChar, indentInc = 2, parent = true, isJson = true) {
		...
		output = '';
		let lineStart = parent ? currentIndent : '\n' + currentIndent;
     ...
     * @param {String} target The string or char to search for
     * @param {String} replacer The string or char to replace the target with
     * @param {Boolean} allInstances Replace all instances?
     * @param {Boolean} logError Log any error that is cough in this process?
     */
