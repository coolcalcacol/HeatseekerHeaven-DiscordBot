...
async function executeCommand(interaction) {
	...
	try { await command.execute(interaction); } catch (error) {
		...
		generalUtilities.actions.handleError().createrr(client, interaction.channel.name, `${interaction.commandName}\`\`\` Executed by: ${interaction.user} \`\`\`User ID: ${interaction.user.id}`, error);
...
async function executeButton(interaction) {
	...
	try { await button.execute(interaction); } catch (error) {
		...
		generalUtilities.actions.handleError().createrr(client, interaction.channel.name, `${interaction.customId}\`\`\` Executed by: ${interaction.user} \`\`\`User ID: ${interaction.user.id}`, error);
		cConsole.log('Error: ' + error.message);
		console.log(error.stack);
	}
}
